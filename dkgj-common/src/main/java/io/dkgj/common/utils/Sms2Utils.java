...
public class Sms2Utils {
    ...
    public static JSONObject sendQdzSms(String mobile, String chName, String code) throws Exception {
        log.info(String.format("发送熊了花短信，参数：{%s,%s,%s}", mobile, chName, code));
        HttpClient httpClient = new HttpClient();
        PostMethod postMethod = new PostMethod("http://api.xunbd.com/sms/single_send");
        postMethod.getParams().setContentCharset("UTF-8");
        postMethod.getParams().setParameter(HttpMethodParams.RETRY_HANDLER, new DefaultHttpMethodRetryHandler());

        String accesskey = "dAhjGUKAGvJbEv2L"; //用户开发key
        String accessSecret = "gSXp1cuw1kQpYGSGIr6omyHq8zeglzn3"; //用户开发秘钥
