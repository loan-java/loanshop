...
public class MakeExcel {
     ...
     * @param headList 很重要，它是列的展示，当然和数据的列要对应不然找不到对应的地方存储
     * @param message  表格第一行的表头信息
     * @throws WriteException
     * @throws IOException
     */
    public static void CreateExcelFile(String fileName, OutputStream outputStream, List<Map<String, Object>> list, List<String> headList,
        ...
        if (list.size() == 0) {
            ...
            for (int j = 0; j < sheetSize; j++) {
                ...
                WritableSheet sheet = book.createSheet(fileName.replace(".xls", "") + j, j);
                ...
                WritableCellFormat header_center = new WritableCellFormat(BoldFont);
                header_center.setBorder(Border.ALL, BorderLineStyle.THIN); // 线条
                header_center.setVerticalAlignment(VerticalAlignment.CENTRE); // 文字垂直对齐
                header_center.setAlignment(Alignment.CENTRE); // 文字水平对齐
                header_center.setWrap(false); // 文字是否换行
                ...
                sheet.addCell(new Label(0, 0, message, header_center));

                // 输出列名
                index = 0;
                for (String name : headList) {
                    // 上面合并了首行，所有这里在第二行开始输出列名
                    sheet.addCell(new Label(index, 1, name, header_center));
                    index++;
                }

                WritableFont normalFont = new WritableFont(WritableFont.createFont("宋体"), 12);
                WritableCellFormat content_center = new WritableCellFormat(normalFont);
                ...
                while (flagList <= list.size()) {
                    ...
                    if (i < 65534) {
                        for (String name : headList) {
                            sheet.addCell(new Label(index, t, list.get(flagList - 1).get(name) + "", content_center));
                            index++;
                        }
                        i++;
                        t++;
