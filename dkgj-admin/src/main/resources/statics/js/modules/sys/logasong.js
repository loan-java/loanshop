...
var vm = new Vue({
	...
	methods: {
		download: function () {
			window.location.href = baseURL + "sys/logasong/export?state=" + vm.q.state + "&createDate=" + $('#qcreatedate').val();
		},
		query: function () {
			vm.reload();
		},
		...
		saveOrUpdate: function (event) {
			var url = vm.logAso.id == null ? "sys/logasong/save" : "sys/logasong/update";
			$.ajax({
				type: "POST",
				url: baseURL + url,
				contentType: "application/json",
				data: JSON.stringify(vm.logAso),
				success: function (r) {
					if (r.code === 0) {
		...
		del: function (event) {
			...
			confirm('确定要删除选中的记录？', function () {
				$.ajax({
					type: "POST",
					url: baseURL + "sys/logasong/delete",
					contentType: "application/json",
		...
		getInfo: function (id) {
			$.get(baseURL + "sys/logasong/info/" + id, function (r) {
				vm.logAso = r.logAso;
			});
		},
